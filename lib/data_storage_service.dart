...
class DataStorageService {
 ...
 Future<void> saveBinderKeychain(String binderId, String keychainPath) async {
  ...
  Future<void> setSelectedFrame(String framePath) async {
  if (_currentUserId == null) {
    debugPrint('❌ Tentativa de atualizar frame sem usuário logado');
    return;
  }
  ...
  try {
    await _supabaseClient
      ...
      .upsert({
        'user_id': _currentUserId,
        'selected_frame': framePath,
        'updated_at': DateTime.now().toIso8601String(),
      });
    
    debugPrint('✅ Frame atualizado com sucesso: $framePath');
  } catch (e) {
    debugPrint('❌ Erro ao atualizar frame no Supabase: $e');
    throw Exception('Falha ao atualizar frame do usuário');
  }
}

  Future<String?> getSelectedFrame() async {
    if (_currentUserId == null) {
      debugPrint('ℹ️ Tentativa de buscar frame sem usuário logado');
      return null;
    }
    ...
    try {
      ...
      if (response == null) {
        debugPrint('ℹ️ Nenhum frame encontrado para $_currentUserId');
        return null;
      }

      return response['selected_frame'] as String?;
    } catch (e) {
      debugPrint('❌ Erro ao buscar frame no Supabase: $e');
      return null;
    }
  }
